@php
    ...
    if ($global_config->bg_images) {
        ...
        if (isset($bg_images_existing['404'])) {
            $bg_img = $bg_images_existing['404'];
        }
    }
@endphp
...
<head>
    ...
    <title>404 ({{ ucwords(lang("page not found")) }}) | {!! $global_config->meta_title !!}</title>
    ...
    @if (isset($global_config->og_type))
        ...
        <meta property="og:title" content="404 ({{ ucwords(lang("page not found")) }}) | {!! $global_config->og_title !!}" />
