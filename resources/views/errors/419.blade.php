@php
    ...
    if ($global_config->bg_images) {
        ...
        if (isset($bg_images_existing['419'])) {
            $bg_img = $bg_images_existing['419'];
        }
    }
@endphp
...
<head>
    ...
    <title>419 ({{ ucwords(lang("page expired")) }}) | {!! $global_config->meta_title !!}</title>
    ...
    @if (isset($global_config->og_type))
        ...
        <meta property="og:title" content="419 ({{ ucwords(lang("page expired")) }}) | {!! $global_config->og_title !!}" />
...
<body class="{{ $preview_class }}">
    ...
    <div class='container'>
      <div class='row content'>
        ...
        <div class='col-lg-12'>
          <h1>419</h1>
          <h2>{{ lang("Sorry, your page has expired.") }}</h2>
          <p>
            {{ lang("You may want to return back.") }}
          </p>
          <a href="{{ $homepage }}" class='btn'>{{ strtoupper(lang("return back")) }}</a>
        </div>
      </div>
    </div>
