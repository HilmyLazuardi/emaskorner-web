...
@php
    $module             = ucwords(lang('blog subscription', $translations));
    $pagetitle          = $module;
    ...
    <script>
        ...
        function refresh_data() {
            ...
            var table = $('#datatables').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.blog_subscription.get_data') }}",
                order: [[ 0, 'asc' ]],
                columns: [
                    { data: 'id', name: 'id' },
        ...
        function confirm_export() {
            if (confirm("{{ lang('Are you sure to export this #item?', $translations, ['#item' => $pagetitle]) }}")) {
                window.location.href = "{{ route('admin.blog_subscription.export') }}";
            }
        }
    </script>
@endsection