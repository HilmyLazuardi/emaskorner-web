...
@php
    ...
    $pagetitle      = ucwords(lang('news category', $translations));
    if (isset($data)) {
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link       = route('admin.news_category.update', $raw_id);
    } else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link       = route('admin.news_category.store');
        $data       = null;
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST" enctype="multipart/form-data">
                            ...
                            @php
                                ...
                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off"';
                                $config->placeholder = 'must be unique. if left empty, system will auto-generate this';
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.news_category') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.news_category.delete') }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
