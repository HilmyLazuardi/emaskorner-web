...
@php
    ...
    $pagetitle      = ucwords(lang('news', $translations));
    if (isset($data)) {
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link       = route('admin.news.update', $raw_id);
    } else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link       = route('admin.news.store');
        $data       = null;
    }
@endphp
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                            ...
                            <div class="x_panel">
                                ...
                                <div class="x_content">
                                    ...
                                    @php
                                        ...
                                        $config = new \stdClass();
                                        $config->defined_data = $news_category;
                                        $config->placeholder = '- '.ucwords(lang('please choose one', $translations)).' -';
                                        echo set_input_form('select2', 'news_category_id', ucwords(lang('news category', $translations)), $data, $errors, true, $config);
                                        ...
                                        $config->path = 'uploads/news/';
                                        $required = true;
                                        if (!empty($data)) {
                                            $required = false;
                                        }
                                        echo set_input_form('image', 'thumbnail', ucwords(lang('thumbnail', $translations)), $data, $errors, $required, $config);
                                        ...
                                        $config->attributes = 'autocomplete="off"';
                                        $config->limit_chars = 100;
                                        $config->placeholder = lang('Penulis', $translations);
                                        echo set_input_form('text', 'author', ucwords(lang('penulis', $translations)), $data, $errors, false, $config);

                                        $config = new \stdClass();
                                        $config->attributes = 'autocomplete="off"';
                                        $config->limit_chars = 100;
                                        $config->placeholder = lang('Lokasi', $translations);
                                        echo set_input_form('text', 'location', ucwords(lang('lokasi', $translations)), $data, $errors, false, $config);

                                        $config = new \stdClass();
                                        $config->attributes = 'readonly';
                                        $config->placeholder = 'dd/mm/yyyy';
                                        echo set_input_form('datepicker', 'posted_at', ucwords(lang('posted date', $translations)), $data, $errors, false, $config);
                                    @endphp
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                            ...
                                            <a href="{{ route('admin.news') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                                @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.news.delete') }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
