...
@php
    ...
    $module = ucwords(lang('office', $translations));
    $pagetitle = $module;
    ...
    <div class="">
        ...
        <div class="page-title">
            ...
            @if (isset($deleted_data))
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.office') }}" class="btn btn-round btn-primary" style="float: right;">
            ...
            @else
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        @if (Helper::authorizing('Office', 'Restore')['status'] == 'true')
                            <a href="{{ route('admin.office.deleted_data') }}" class="btn btn-round btn-danger" style="float: right; margin-bottom: 5px;" data-toggle="tooltip" title="{{ ucwords(lang('view deleted items', $translations)) }}">
                                <i class="fa fa-trash"></i>
                            </a>
                        @endif
                        @if (Helper::authorizing('Office', 'Add New')['status'] == 'true')
                            <a href="{{ route('admin.office.create') }}" class="btn btn-round btn-success" style="float: right;">
                    ...
                    <div class="x_content">
                        <div class="table-responsive">
                            <table id="datatables" class="table table-striped table-bordered dt-responsive" style="display:none">
                                <thead>
                                    <tr>
                                        <th>{{ ucwords(lang('office', $translations)) }}</th>
...
@section('script')
    ...
    <script>
        var AjaxSortingURL = '{{ route("admin.office.sorting") }}';
        ...
        function refresh_data() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('admin.office.get_data') }}",
        ...
        function refresh_deleted_data() {
            ...
            var table = $('#datatables-deleted').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.office.get_deleted_data') }}",
                order: [[ 0, 'asc' ]],
                columns: [
                    {data: 'name', name: 'name'},
