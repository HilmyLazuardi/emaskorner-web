...
@php
    ...
    $pagetitle = ucwords(lang('language', $translations)) . ' - ' . $parent_data->country_name;
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.language.update', [$parent_raw_id, $raw_id]);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.language.store', $parent_raw_id);
        $data = null;
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST" enctype="multipart/form-data">
                            ...
                            @php
                                ...
                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off" maxlength="20"';
                                $config->placeholder = 'sample: English';
                                echo set_input_form('text', 'name', ucwords(lang('name', $translations)), $data, $errors, true, $config);

                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off" maxlength="10"';
                                $config->placeholder = 'sample: EN';
                                echo set_input_form('capital', 'alias', ucwords(lang('alias', $translations)), $data, $errors, true, $config);
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.language.delete', $parent_raw_id) }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
