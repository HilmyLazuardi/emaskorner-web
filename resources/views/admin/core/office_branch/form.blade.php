...
@php
    ...
    $pagetitle = ucwords(lang('branch office', $translations));
    $pagetitle .= ' - ' . $parent_data->name; 
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.office_branch.update', [$parent_id, $raw_id]);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.office_branch.store', $parent_id);
        $data = null;
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST" enctype="multipart/form-data">
                            ...
                            @php
                                ...
                                echo set_input_form('number_only', 'fax', ucwords(lang('fax', $translations)), $data, $errors, false, $config);
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.office_branch', $parent_id) }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.office_branch.delete', $parent_id) }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
