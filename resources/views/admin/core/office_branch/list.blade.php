...
@php
    $module = ucwords(lang('branch office', $translations));
    $pagetitle = $module . ' - ' . $data->name;
    ...
    <div class="">
        ...
        <div class="page-title">
            ...
            @if (isset($deleted_data))
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.office_branch', $raw_id) }}" class="btn btn-round btn-primary" style="float: right;">
            ...
            @else
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.office_branch.deleted_data', $raw_id) }}" class="btn btn-round btn-danger" style="float: right; margin-bottom: 5px;" data-toggle="tooltip" title="{{ ucwords(lang('view deleted items', $translations)) }}">
                            <i class="fa fa-trash"></i>
                        </a>
                        <a href="{{ route('admin.office_branch.create', $raw_id) }}" class="btn btn-round btn-success" style="float: right;">
                            <i class="fa fa-plus-circle"></i>&nbsp; {{ ucwords(lang('add new', $translations)) }}
                        </a>
                        <a href="{{ route('admin.office') }}" class="btn btn-round btn-info" style="float: right; margin-bottom: 5px;" data-toggle="tooltip" title="{{ ucwords(lang('back', $translations)) }}">
                    ...
                    <div class="x_content">
                        <div class="table-responsive">
                            <table id="datatables" class="table table-striped table-bordered dt-responsive" style="display:none">
                                <thead>
                                    <tr>
                                        <th>{{ ucwords(lang('branch office', $translations)) }}</th>
                                        <th>{{ ucwords(lang('phone', $translations)) }}</th>
...
@section('script')
    ...
    <script>
        var AjaxSortingURL = '{{ route("admin.office_branch.sorting", $raw_id) }}';
        ...
        function refresh_data() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('admin.office_branch.get_data', $raw_id) }}",
        ...
        function refresh_deleted_data() {
            ...
            var table = $('#datatables-deleted').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.office_branch.get_deleted_data', $raw_id) }}",
                order: [[ 0, 'asc' ]],
                columns: [
                    {data: 'name', name: 'name'},
