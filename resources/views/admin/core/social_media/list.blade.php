...
@php
    $module = ucwords(lang('social media', $translations));
    $pagetitle = $module;
    ...
    <div class="">
        ...
        <div class="page-title">
            ...
            @if (isset($deleted_data))
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.social_media') }}" class="btn btn-round btn-primary" style="float: right;">
            ...
            @else
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.social_media.deleted_data') }}" class="btn btn-round btn-danger" style="float: right; margin-bottom: 5px;" data-toggle="tooltip" title="{{ ucwords(lang('view deleted items', $translations)) }}">
                            <i class="fa fa-trash"></i>
                        </a>
                        <a href="{{ route('admin.social_media.create') }}" class="btn btn-round btn-success" style="float: right;">
                            ...
                            <table id="datatables" class="table table-striped table-bordered dt-responsive" style="display:none">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ ucwords(lang('logo', $translations)) }}</th>
                                        <th>{{ ucwords(lang('link', $translations)) }}</th>
...
@section('script')
    ...
    <script>
        var AjaxSortingURL = '{{ route("admin.social_media.sorting") }}';
        ...
        function refresh_data() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('admin.social_media.get_data') }}",
                success: function(response){
                    if (typeof response.status != 'undefined') {
                        if (response.status == 'true') {
                            var html = '';
                            if (response.data == '') {
                                html += '<tr><td colspan="7"><h2 class="text-center">{{ strtoupper(lang("no data available", $translations)) }}</h2></td></tr>';
                            } else {
                                $.each(response.data, function (index, value) {
                                    html += '<tr role="row" id="row-'+value.id+'" title="{{ lang("Drag & drop to sorting", $translations) }}" data-toggle="tooltip">';
                                        ...
                                        html += '<td><img src="'+value.image_item+'" style="max-width:100px;"></td>';
        ...
        function refresh_deleted_data() {
            ...
            var table = $('#datatables-deleted').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.social_media.get_deleted_data') }}",
                order: [[ 0, 'asc' ]],
                columns: [
                    {data: 'name', name: 'name'},
                    {data: 'link', name: 'link'},
