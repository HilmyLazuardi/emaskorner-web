...
@php
    ...
    $pagetitle = ucwords(lang('blocked IP', $translations));
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.blocked_ip.update', $raw_id);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.blocked_ip.store');
        $data = null;
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST">
                            ...
                            @php
                                ...
                                $config->placeholder = '192.168.212.1';
                                echo set_input_form('text', 'ip_address', ucwords(lang('IP address', $translations)), $data, $errors, true, $config);
                                
                                if ($data) {
                                    $config = new \stdClass();
                                    $config->attributes = 'readonly';
                                    echo set_input_form('text', 'username', ucwords(lang('admin', $translations)), $data, $errors, false, $config);
                                }
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.blocked_ip') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.blocked_ip.delete') }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
