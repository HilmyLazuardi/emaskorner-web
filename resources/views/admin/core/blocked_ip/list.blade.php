...
@php
    $module = ucwords(lang('blocked IP', $translations));
    $pagetitle = $module;
    ...
    <div class="">
        ...
        <div class="page-title">
            ...
            @if (isset($deleted_data))
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.blocked_ip') }}" class="btn btn-round btn-primary" style="float: right;">
            ...
            @else
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        <a href="{{ route('admin.blocked_ip.deleted_data') }}" class="btn btn-round btn-danger" style="float: right; margin-bottom: 5px;" data-toggle="tooltip" title="{{ ucwords(lang('view deleted items', $translations)) }}">
                            <i class="fa fa-trash"></i>
                        </a>
                        <a href="{{ route('admin.blocked_ip.create') }}" class="btn btn-round btn-success" style="float: right;">
                        ...
                        <div class="table-responsive">
                            <table id="datatables" class="table table-striped table-bordered dt-responsive" style="display:none">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>{{ ucwords(lang('IP address', $translations)) }}</th>
                                        <th>{{ ucwords(lang('admin', $translations)) }}</th>
    ...
    <script>
        ...
        function refresh_data() {
            ...
            var table = $('#datatables').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.blocked_ip.get_data') }}",
                order: [[ 0, 'desc' ]],
                columns: [
                    {data: 'id', name: 'id'},
        ...
        function refresh_deleted_data() {
            ...
            var table = $('#datatables-deleted').DataTable({
                ...
                serverSide: true,
                processing: true,
                ajax: "{{ route('admin.blocked_ip.get_deleted_data') }}",
                order: [[ 0, 'desc' ]],
                columns: [
                    {data: 'id', name: 'id'},
                    {data: 'ip_address', name: 'ip_address'},
