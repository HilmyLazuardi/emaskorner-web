...
{{-- SMALL MODAL CONFIG --}}
@section('small_modal_id', 'modal_add_item')
@section('small_modal_title', ucwords(lang('add #item', $translations, ['#item'=>ucwords(lang('item', $translations))])))
@section('small_modal_content')
    <script>
        var form_type = '{{ $type }}';
        var enable_other_option = {{ $enable_other_option }};
    ...
    {{-- add_question(
        ...
        collapsed = false,
        identifier = 0) --}}
    <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'multiple_choice', 'text', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-bullseye"></i>&nbsp; Multiple Choice (Text)</span><br>
    <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'multiple_choice', 'image', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-bullseye"></i>&nbsp; Multiple Choice (Text & Image)</span><br>
    <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'drop-down', 'text', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-list"></i>&nbsp; Drop-down</span><br>
    @if ($type == 'questionnaire')
        <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'checkboxes', 'text', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-check-square-o"></i>&nbsp; Checkboxes (Text)</span><br>
        <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'checkboxes', 'image', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-check-square-o"></i>&nbsp; Checkboxes (Text & Image)</span><br>
        <span class="btn btn-primary btn-block" onclick="add_question(form_type, 'linear_scale', 'text', '', '', '', enable_other_option, collapsed, 0)"><i class="fa fa-list-ol"></i>&nbsp; Linear Scale</span><br>
    @endif
@endsection
...
@php
    ...
    $pagetitle = ucwords(lang('form', $translations)) . ' ' . ucwords(lang($type, $translations));
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.form.update', $raw_id);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.form.store', $type);
        $data = null;
    }
@endphp
...
@section('content')
                    ...
                    <div class="x_panel">
                        ...
                        <div class="x_content">
                            ...
                            @php
                                ...
                                // $config = new \stdClass();
                                // $config->attributes = 'autocomplete="off"';
                                // echo set_input_form('text', 'intro_title', ucwords(lang('introduction title', $translations)), $data, $errors, false, $config);

                                $config = new \stdClass();
                                $config->autosize = true;
                                echo set_input_form('textarea', 'intro_description', ucwords(lang('introduction description', $translations)), $data, $errors, false, $config);

                                // $config = new \stdClass();
                                // $config->defined_data = ['none','image','video'];
                                // $config->attributes = 'onchange="show_media()"';
                                // echo set_input_form('select', 'intro_media', ucwords(lang('introduction media', $translations)), $data, $errors, false, $config);

                                $config = new \stdClass();
                                $config->popup = true;
                                $config->info = '<i class="fa fa-info-circle"></i>&nbsp; '.lang('Only support #allowed_ext, MAX #size', $translations, ['#allowed_ext'=>'jpg/jpeg/png', '#size'=>'2MB']);
                                echo set_input_form('image', 'intro_src_image', ucwords(lang('introduction media - image', $translations)), $data, $errors, true, $config);

                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off"';
                                $config->placeholder = 'https://www.youtube.com/watch?v=XXXX';
                                echo set_input_form('text', 'intro_src_video', ucwords(lang('introduction media - video', $translations)), $data, $errors, true, $config);

                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off"';
                                $config->placeholder = '0';
                                $config->info_text = '<i class="fa fa-info-circle"></i>&nbsp; '.lang('User will get this point after submit this form, usually used for questionnaire but not required.', $translations);
                                echo set_input_form('number_format', 'form_point', ucwords(lang('amount point', $translations)), $data, $errors, false, $config);

                                // $config = new \stdClass();
                                // $config->default = 'checked';
                                // $config->info_text = '<i class="fa fa-info-circle"></i>&nbsp; '.lang('User must complete all this form to submit.', $translations);
                                // echo set_input_form('switch', 'must_complete', ucfirst(lang('must complete all to submit', $translations)), $data, $errors, false, $config);
                            @endphp

                            <div class="ln_solid"></div>
                            
                            @php
                                echo set_input_form('switch', 'status', ucwords(lang('published status', $translations)), $data, $errors, false);

                                $config = new \stdClass();
                                $config->placeholder = 'dd/mm/yyyy hh:mm';
                                echo set_input_form('datetimepicker', 'started_at', ucwords(lang('start from', $translations)), $data, $errors, false, $config);
                                echo set_input_form('datetimepicker', 'finished_at', ucwords(lang('until to', $translations)), $data, $errors, false, $config);
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <span class="btn btn-primary" data-toggle="modal" data-target="#modal_add_item">
                                        <i class="fa fa-plus-circle"></i> {{ ucwords(lang('add #item', $translations, ['#item'=>ucwords(lang('item', $translations))])) }}
                                    </span>
                                </div>
                                <div class="col-lg-12" style="margin: 5px 0;">
                                    <div id="content-pagebuilder" class="accordion"></div>
                                </div>
                                <div class="col-lg-12" id="btn_add_item_bottom" style="display: none">
                                    <span class="btn btn-primary" data-toggle="modal" data-target="#modal_add_item">
                                        <i class="fa fa-plus-circle"></i> {{ ucwords(lang('add #item', $translations, ['#item'=>ucwords(lang('item', $translations))])) }}
                                    </span>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.form') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                                    ...
                                    @if (isset($raw_id))
                                        {{-- |&nbsp; <a href="{{ route('web.form', $data->slug) }}?preview={{ date('YmdHis') }}" target="_blank" class="btn btn-warning" onclick="return confirm_preview()"><i class="fa fa-external-link"></i>&nbsp; {{ ucwords(lang('preview', $translations)) }}</a> --}}
    ...
    @if (isset($raw_id))
        <form id="form_delete" action="{{ route('admin.form.delete') }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
            @csrf
...
@section('script')
    ...
    <!-- KINIDI Tech - PageBuilder (Form) -->
    @include('_vendors.pagebuilder.script_form')

    <script>
        // KINIDI Tech - PageBuilder - Config (BEGIN)
        // SET PAGEBUILDER MODE
        var pagebuilder_mode = 'form';
        ...
        $( document ).ready(function() {
            // KINIDI Tech - PageBuilder (Form) - Content Loader (BEGIN)
            @include('_vendors.pagebuilder.script_form_helper')
            // KINIDI Tech - PageBuilder (Form) - Content Loader (END)
        });
        ...
        function show_media() {
            $('.vinput_intro_src_image').hide();
            $('#intro_src_image').prop('required', false);
            $('.vinput_intro_src_video').hide();
            $('#intro_src_video').prop('required', false);
            
            var media_type = $('#intro_media').val();
            switch (media_type) {
                case 'image':
                    @if (empty($data))
                        $('#intro_src_image').prop('required', true);
                    @endif
                    $('.vinput_intro_src_image').show();
                    break;

                case 'video':
                    $('#intro_src_video').prop('required', true);
                    $('.vinput_intro_src_video').show();
                    break;
        ...
        function input_point() {
            var form_type = '{{ $type }}';
            $('.vinput_form_point').hide();
            if (form_type == 'questionnaire') {
                $('.vinput_form_point').show();
            }
        }
    </script>
@endsection