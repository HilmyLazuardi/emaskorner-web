...
@php
    ...
    $pagetitle = ucwords($position) . ' ' . ucwords(lang('navigation sub menu', $translations)) . ' (Lvl '.($parent_data->level + 1).') - ' . $parent_data->name;
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.nav_menu_child.update', [$position, $raw_parent, $raw_id]);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.nav_menu_child.store', [$position, $raw_parent]);
        $data = null;
    }
@endphp
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.nav_menu_child', [$position, $raw_parent]) }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.nav_menu.delete', [$position, $raw_id]) }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
