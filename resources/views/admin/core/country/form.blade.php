...
@php
    ...
    $pagetitle = ucwords(lang('country', $translations));
    if(isset($data)){
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link = route('admin.country.update', $raw_id);
    }else {
        $pagetitle .= ' ('.ucwords(lang('new', $translations)).')';
        $link = route('admin.country.store');
        $data = null;
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST">
                            ...
                            @php
                                ...
                                $config->placeholder = 'sample: Indonesia';
                                echo set_input_form('capital', 'country_name', ucwords(lang('country name', $translations)), $data, $errors, true, $config);
                                
                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off" maxlength="2"';
                                $config->placeholder = 'sample: ID';
                                echo set_input_form('capital', 'country_alias', ucwords(lang('country alias', $translations)), $data, $errors, true, $config);
                                
                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off" maxlength="3"';
                                $config->placeholder = 'sample: IDN';
                                echo set_input_form('capital', 'iso3', 'ISO3', $data, $errors, false, $config);

                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off"';
                                $config->placeholder = 'sample: 360';
                                echo set_input_form('number_only', 'numcode', ucwords(lang('number code', $translations)), $data, $errors, false, $config);
                                
                                $config = new \stdClass();
                                $config->attributes = 'autocomplete="off"';
                                $config->placeholder = 'sample: 62';
                                echo set_input_form('number_only', 'country_code', ucwords(lang('country code', $translations)), $data, $errors, true, $config);
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{ route('admin.country') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data))
                        ...
                        @if (isset($raw_id))
                            <form id="form_delete" action="{{ route('admin.country.delete') }}" method="POST" onsubmit="return confirm('{!! lang('Are you sure to delete this #item?', $translations, ['#item' => 'data']) !!}');" style="display: none">
                                @csrf
