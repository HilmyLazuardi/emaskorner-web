...
@php
    ...
    $pagetitle = ucwords(lang('Electronic Contract', $translations));
    if (isset($data)) {
        $pagetitle .= ' ('.ucwords(lang('edit', $translations)).')';
        $link       = route('admin.econtract.update', $raw_id);
    }
@endphp
                    ...
                    <div class="x_content">
                        ...
                        <form id="form_data" class="form-horizontal form-label-left" action="{{ $link }}" method="POST" enctype="multipart/form-data">
                            ...
                            @php
                                ...
                                $config             = new \stdClass();
                                $required           = true;
                                $config->value  = asset($data->document);
                                echo set_input_form('file', 'document', ucwords(lang('document', $translations)), $data, $errors, $required, $config);
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    <button type="submit" class="btn btn-success"><i class="fa fa-save"></i>&nbsp; 
                                        @if (isset($data)) {{ ucwords(lang('save', $translations)) }} @else {{ ucwords(lang('submit', $translations)) }} @endif
                                    </button>

                                    <a href="{{ route('admin.econtract') }}" class="btn btn-default"><i class="fa fa-times"></i>&nbsp; 
                                        @if (isset($data)) {{ ucwords(lang('close', $translations)) }} @else {{ ucwords(lang('cancel', $translations)) }} @endif
                                    </a>
                                </div>
                            </div>
...
@section('script')
    ...
    <script>
        $('#master_check_all').on("click", function() {
            var all     = $('.check_service').length;
            var total   = $('.check_service:checked').length;
